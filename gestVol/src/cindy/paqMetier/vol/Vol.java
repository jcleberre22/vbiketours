...
import cindy.paqMetier.comm.IVol;
...
public class Vol implements IVol {
	 ...
	 */
	public Vol(String reference, String circulation, int categorieDeVol,List<SortieAerienne> laliste,GregorianCalendar dateDecollage,GregorianCalendar dateAtterrissage) {
	...
	public void setDecollage(GregorianCalendar decollage) {
		if(decollage == null){
			throw new RuntimeException("Vol : la date de decollage ne peut être null");
		}
		else
	...
	public void setAtterrissage(GregorianCalendar atterrissage) {
		if(atterrissage == null){
			throw new RuntimeException("Vol : la date d'atterrissage ne peut être null");
		}
		else
	...
	private void setLaCategorie(int categorieDeVol) {
		if(categorieDeVol < 0)
			throw new RuntimeException("les catégories de vol négatives ne sont pas acceptées");
	...
	public void ajouterSortieAerienne(SortieAerienne laSortie) {
		if(laSortie == null)
			throw new RuntimeException("la sortie passée en paramètre est nulle");
	...
	public void modifierSortieAerienne(SortieAerienne old,
		...
		if(old != null && newSortie != null){
			if (lesSorties.contains(old)){
				...
				throw new RuntimeException("La sortie demandée n'est pas contenue dans la liste");
		}
		else{
			throw new RuntimeException("l'ancienne ou la nouvelle sortie n'existe pas");
		}
	}
	...
	public List<SortieAerienne> consulterSortieAerienne(GregorianCalendar debut,GregorianCalendar fin){
		List<SortieAerienne> filtre = new ArrayList<SortieAerienne>();
			for (int i = 0; i < lesSorties.size(); i++) {
				if(lesSorties.get(i).getDecolage().after(debut) || lesSorties.get(i).getAtterissage().before(fin)){
